...
void launch_gpu_mining(
    ...
    // Dispatch to the best kernel based on tile dimensions.
    // WMMA (Tensor Core) kernels are available for 16x16, 32x32, 64x64,
    // and 128x128 tiles. These use mma.sync instructions on SM80+ (Ada/Blackwell)
    // and provide ~4x throughput vs. the scalar DP4A fallback.
    // For all other tile sizes, use the scalar fallback kernel.
    bool wmma_selected = false;

    if (tile_h == 16 && tile_w == 16) {
        printf("[pearl_gemm] launch decision: FUSED WMMA 16x16 path selected\n");
        launch_gpu_mining_wmma_16(
        ...
        wmma_selected = true;
    } else if (tile_h == 32 && tile_w == 32) {
        printf("[pearl_gemm] launch decision: FUSED WMMA 32x32 path selected\n");
        launch_gpu_mining_wmma_32(
            reinterpret_cast<const int8_t*>(a_noised),
            reinterpret_cast<const int8_t*>(b_noised_t),
            a_rows_data, b_cols_data,
            jobs, jackpots, hashes, winner_flags,
            num_candidates, P, Q, m, n, k, rank, stream);
        wmma_selected = true;
    } else if (tile_h == 64 && tile_w == 64) {
        printf("[pearl_gemm] launch decision: FUSED WMMA 64x64 path selected\n");
        launch_gpu_mining_wmma_64(
            reinterpret_cast<const int8_t*>(a_noised),
            reinterpret_cast<const int8_t*>(b_noised_t),
            a_rows_data, b_cols_data,
            jobs, jackpots, hashes, winner_flags,
            num_candidates, P, Q, m, n, k, rank, stream);
        wmma_selected = true;
    } else if (tile_h == 128 && tile_w == 128) {
        printf("[pearl_gemm] launch decision: FUSED WMMA 128x128 path selected\n");
        launch_gpu_mining_wmma_128(
            reinterpret_cast<const int8_t*>(a_noised),
            reinterpret_cast<const int8_t*>(b_noised_t),
            a_rows_data, b_cols_data,
            jobs, jackpots, hashes, winner_flags,
    ...
    if (!wmma_selected) {
        // Scalar fallback for non-16×16, 32×32, 64×64, or 128×128 tiles.
        printf("[pearl_gemm] launch decision: SCALAR fallback selected (gpu_mining_kernel)\n");
