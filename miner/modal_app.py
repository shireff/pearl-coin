...
REPO_URL = "https://github.com/shireff/pearl-cion.git"
...
image = (
    modal.Image.from_registry("nvidia/cuda:12.6.2-runtime-ubuntu22.04", add_python="3.12")
    .apt_install("git", "curl", "build-essential", "pkg-config", "libssl-dev", "clang", "lld")
    .pip_install("uv")
    .pip_install("torch==2.11.0", index_url="https://download.pytorch.org/whl/cu126")
    .pip_install("numpy")
    .env({
    ...
    .run_commands(
        ...
        'curl -fsSL https://go.dev/dl/go1.26.0.linux-amd64.tar.gz -o /tmp/go.tar.gz && tar -C /usr/local -xzf /tmp/go.tar.gz',
        f'git clone --recurse-submodules {REPO_URL} {CLONE_DIR}',
        f'cd {CLONE_DIR}/xmss && make',
        f'mkdir -p {CLONE_DIR}/zk-pow/src/circuit && touch {CLONE_DIR}/zk-pow/src/circuit/v2_cache.bin',
        f'mkdir -p {CLONE_DIR}/zk-pow/src/v1 && touch {CLONE_DIR}/zk-pow/src/v1/v1_cache.bin',
        f'cd {CLONE_DIR}/zk-pow/bindings/go && cargo build --release',
        f'cd {CLONE_DIR} && go build -tags xmss,zkpow -o bin/pearld ./node',
        f'cd {CLONE_DIR} && go build -tags xmss,zkpow -o bin/prlctl ./node/cmd/prlctl',
        # pearld looks for this template next to its own binary to generate a
        # default config on first run. It doesn't ship one, so we stub it out
        # to silence the "no such file or directory" warning at startup.
        f'touch {CLONE_DIR}/bin/sample-pearld.conf',
        f'mkdir -p /root/.pearl',
        'curl -LsSf https://astral.sh/uv/install.sh | sh',
        f'cd {CLONE_DIR} && uv sync --all-packages',
    )
)
...
def _print_startup_banner():
    print("=" * 70)
    print("PEARL MINER — Production Profile")
    print("=" * 70)
...
def run_mining():
    ...
    try:
        ...
        # Without this, a failure here surfaces to the local client only as an
        # opaque "RemoteError" with no message. Print the full traceback into
        # the container logs before it propagates so it's actually debuggable.
...
def main():
    # .remote() blocks and ties the remote container's lifetime to this local
    # process staying connected — if your terminal loses focus, the machine
    # sleeps, or the network blips, Modal sends SIGINT to everything in the
    # container and you get exactly the "Received signal (interrupt)" +
    # bare RemoteError you saw. .spawn() detaches: the function keeps running
    # on Modal's infra regardless of what happens locally.
    call = run_mining.spawn()
    print(f"Mining job started, call id: {call.object_id}")
    print("This process can now exit safely — the job keeps running remotely.")
    print(f"Reattach later with: modal.FunctionCall.from_id('{call.object_id}').get()")